...
def split_items(value: str | None) -> list[str]:
    if value is None:
        return []
    parts = [part.strip() for part in SOURCE_SPLIT_RE.split(value)]
...
def parse_internal_evidence_items(items_raw: list[str]) -> tuple[dict[str, str], list[str]]:
    items: dict[str, str] = {}
    invalid: list[str] = []
    for raw in items_raw:
...
def parse_external_evidence_items(items_raw: list[str]) -> tuple[dict[str, tuple[str, str]], list[str]]:
...
def parse_fact_items(items_raw: list[str]) -> tuple[dict[str, str], list[str]]:
    items: dict[str, str] = {}
    invalid: list[str] = []
    for raw in items_raw:
...
def main() -> int:
    ...
    external_source_items = split_items(external_sources)
    external_evidence_items, invalid_external_items = parse_external_evidence_items(external_source_items)
    ...
    internal_evidence_raw_items = split_items(internal_evidence)
    ...
    if internal_evidence in {None, "", "无"}:
        ...
        internal_evidence_items, invalid_internal_items = parse_internal_evidence_items(internal_evidence_raw_items)
    ...
    fact_raw_items = split_items(verified_facts)
    fact_text_by_id, invalid_fact_items = parse_fact_items(fact_raw_items)
    ...
    if evidence_mapping in {None, "", "无"}:
        ...
        mapping_raw_items = split_items(evidence_mapping)
        ...
        if not parsed_mappings:
            ...
            for fact_id, evidence_id, keyword, excerpt in parsed_mappings:
                ...
                if fact_text is None:
                    ...
                    continue
                expected_fact_text = f"证据摘录：{excerpt}"
                if fact_text != expected_fact_text:
                    errors.append(
                        f"Verified fact {fact_id} must exactly match '证据摘录：{excerpt}' so unsupported extra claims cannot pass."
                    )
